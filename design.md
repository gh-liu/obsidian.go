...
- 在普通位置查询当前文件的反向链接。
- 在 heading 上查询指向该 heading 的引用。
- 可按 LSP `includeDeclaration` 决定是否包含 heading 自身声明。
...
| **Find References** | `textDocument/references` | 若光标在 heading 上 → 扫描全 vault 所有 link，匹配 `target + anchor`；否则 → 扫描全 vault 所有 link，匹配 target 指向当前文件 |
...
| **CodeLens** | `textDocument/codeLens` | 在 frontmatter `id` 和 heading 上显示引用计数 |
